    ...
    private static ImageUrlsBean getImageUrlsBean(IllustsBean illust, int index, String imageResolution) {
        if (illust.getPage_count() == 1) {
            ...
            if (mp == null || mp.isEmpty() || index < 0 || index >= mp.size()) {
                Log.w(
                    "V3MultiP",
                    "[IllustDownload.getImageUrlsBean] SUSPECT: illustId=" + illust.getId() + " page_count=" + illust.getPage_count() + " index=" + index +
                        " meta_pages=" + (mp == null ? "null" : ("size=" + mp.size())) + " — about to throw on .get(index)"
                );
            }
